...
class main(commands.Cog):
    ...
    @slash_command(description="反應問題")
    async def bug(self,ctx,bug):
        channel = await self.bot.fetch_channel(1057901461427736616) #抓取要傳送至的頻道
        embed=discord.Embed(title="問題反應",description=bug)
        embed.set_footer(ctx.author)
        await channel.send(embed=embed) #傳送訊息至指定頻道
        await ctx.respond("已送出問題反饋")
